 ...
 * Cron a cada 2 minutos: verifica se há nomes LOTR em FeedbackRecord
 * e restaura os nomes corretos se necessário.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Buscar todos os FeedbackRecords
    const allRecords = await base44.asServiceRole.entities.FeedbackRecord.list();

    // Verificar se há algum nome LOTR
    const contaminated = allRecords.filter(r =>
      LOTR_NAMES.has(r.employee_name) || LOTR_NAMES.has(r.manager_name)
    );

    if (contaminated.length === 0) {
      return Response.json({ clean: true, message: 'Nenhum nome LOTR encontrado' });
    }
    ...
    await base44.asServiceRole.entities.SecurityLog.create({
      ...
      page: 'system/cron',
      affected_ids: contaminated.map(r => r.id),
      new_value: `Restaurados: ${restored}, Erros: ${errors.length}`
    });

    return Response.json({ restored, contaminated: contaminated.length, errors });
  } catch (error) {
    console.error('checkAndRestoreFeedbackRecord error:', error);
