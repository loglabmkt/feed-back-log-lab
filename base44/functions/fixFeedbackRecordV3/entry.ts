/**
 * fixFeedbackRecordV3
 * Restaura employee_name e manager_name dos 52 FeedbackRecords ativos.
 * Compara o valor atual com o esperado — atualiza apenas se houver diferença.
 * Não depende de lista de nomes fictícios.
 */
...
Deno.serve(async (req) => {
  try {
    ...
    for (const [id, expected] of Object.entries(CORRECTIONS)) {
      try {
        const record = await base44.asServiceRole.entities.FeedbackRecord.get(id);
        if (!record) {
          errors.push({ id, error: `Entity FeedbackRecord with ID ${id} not found` });
          continue;
        }

        if (record.employee_name === expected.employee_name && record.manager_name === expected.manager_name) {
          skipped++;
          continue;
        }

        await base44.asServiceRole.entities.FeedbackRecord.update(id, {
          employee_name: expected.employee_name,
          manager_name: expected.manager_name
        });
        updated++;
      } catch (e) {
