...
Deno.serve(async (req) => {
  try {
    ...
    // Carregar todos os gestores para montar mapa manager_id -> full_name
    const gestores = await base44.asServiceRole.entities.Gestor.list();
    const gestorMap = {};
    for (const g of gestores) {
      gestorMap[g.id] = g.full_name;
    }

    // Carregar todos os FeedbackRecords
    const records = await base44.asServiceRole.entities.FeedbackRecord.list();
    ...
    for (const record of records) {
      const correctedEmployee = EMPLOYEE_CORRECTIONS[record.employee_id];
      const correctedManager = record.manager_id ? gestorMap[record.manager_id] : null;

      const needsEmployeeUpdate = correctedEmployee && correctedEmployee !== record.employee_name;
      const needsManagerUpdate = correctedManager && correctedManager !== record.manager_name;

      if (!needsEmployeeUpdate && !needsManagerUpdate) {
        skipped++;
        continue;
      }

      const patch = {};
      if (needsEmployeeUpdate) patch.employee_name = correctedEmployee;
      if (needsManagerUpdate) patch.manager_name = correctedManager;

      try {
        await base44.asServiceRole.entities.FeedbackRecord.update(record.id, patch);
        updated++;
      } catch (e) {
