...
export default function GestorCadastro() {
  ...
  const handleCheckEmail = async (e) => {
    ...
    try {
      const response = await base44.functions.invoke('checkGestorEmail', { email });
      
      if (response.data.found) {
        setGestorData(response.data.gestor);
        setStep(2);
      } else {
        setError("Email não encontrado no sistema. Entre em contato com o administrador.");
      }
    } catch (err) {
            ...
            <form onSubmit={handleCreatePassword} className="space-y-4">
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg mb-4">
                ...
                <p className="text-sm text-green-600 mt-1">
                  Olá, <strong>{gestorData.full_name}</strong>
                </p>
              </div>
