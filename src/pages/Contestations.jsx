        ...
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {selectedJustification && (
            <div className="space-y-6 py-4">
              <div className="flex items-center gap-4 p-4 bg-red-50 rounded-xl">
                ...
                <div>
                  <p className="text-lg font-semibold text-slate-900">{selectedJustification.employee_name}</p>
                  <p className="text-sm text-slate-500">
                    Contestação em {selectedJustification.created_date && format(new Date(selectedJustification.created_date), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
                  </p>
                </div>
              ...
              <div>
                <p className="text-sm font-semibold text-slate-700 mb-2">Motivo da Contestação</p>
              ...
              {selectedFeedback && (
                <div className="space-y-4 p-4 bg-slate-50 rounded-xl">
                  <p className="text-sm font-semibold text-slate-700">Feedback Original</p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-slate-900">
                        {selectedFeedback.feedback_date && format(new Date(selectedFeedback.feedback_date), "dd/MM/yyyy")}
                      </p>
                    </div>
                  </div>

                  <div>
                    <p className="text-xs text-slate-500 mb-1">Pontos Fortes</p>
                    <p className="text-sm text-slate-700">{selectedFeedback.strengths}</p>
                  </div>

                  <div>
                    <p className="text-xs text-slate-500 mb-1">Pontos de Melhoria</p>
                    <p className="text-sm text-slate-700">{selectedFeedback.improvements}</p>
                  </div>

                  {selectedFeedback.action_plan && (
                    <div>
                      <p className="text-xs text-slate-500 mb-1">Plano de Ação</p>
                      <p className="text-sm text-slate-700">{selectedFeedback.action_plan}</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Adicione suas notas de análise..."
                  className="min-h-24"
                  disabled={selectedJustification.status === 'resolved'}
                />
              </div>

              {selectedJustification.review_date && (
                <div className="p-4 bg-emerald-50 rounded-xl">
                  <p className="text-xs text-emerald-600 mb-1">Análise realizada em</p>
                  <p className="text-sm text-emerald-700">
                    {format(new Date(selectedJustification.review_date), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR })}
                  </p>
                </div>
              )}
            </div>
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            {selectedJustification?.status !== 'resolved' && (
              <>
                {selectedJustification?.status === 'open' && (
                  <Button 
                    variant="outline"
                ...
                >
                  ...
                  {processing ? "Processando..." : "Marcar como Resolvido"}
                </Button>
              </>
            )}
          </DialogFooter>
