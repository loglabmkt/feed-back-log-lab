...
export default function ColaboradorLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await base44.functions.invoke('colaboradorLogin', {
        email,
        password
      });

      if (response.data.success) {
        localStorage.setItem('colaborador_session', JSON.stringify(response.data.colaborador));
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{background: 'linear-gradient(135deg, #14141E 0%, #1a1a2e 100%)'}}>
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1 pb-6">
          ...
          <CardTitle className="text-2xl text-center">Portal do Colaborador</CardTitle>
          <p className="text-center text-slate-500 text-sm">
            Acesse sua conta para visualizar seus feedbacks
          </p>
        </CardHeader>
        <CardContent>
          {error && (
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-center space-y-2">
              <a href="/colaboradorcadastro" className="text-sm text-slate-500 hover:underline block">
